...
namespace CreateEntityModel.AddDatabase.SqlServer.DAL
{
    public class SqlServer
    {
        ...
        public List<TableInfo> GetTableInfo()
        {
            ...
            using (DataTable dt = conn.GetSchema("Tables"))
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        o.Description = Description.Where(p => p.ColumnName.Equals(o.ColumnName)).Select(p => p.Description).FirstOrDefault();
                    });
        ...
        private List<ColumnInfo> GetDescription(string tableName)
        {
            ...
            string sql = string.Format("SELECT  a.name AS ColumnName, isnull(g.[value],'') AS Description FROM  sys.columns a left join sys.extended_properties g  on (a.object_id = g.major_id AND g.minor_id = a.column_id) WHERE  object_id =(SELECT object_id FROM sys.tables WHERE name = '{0}') and g.[value] is not null", tableName);
